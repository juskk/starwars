                    ...
                    for (let item in inside2) {
                        if (inside2[item] instanceof Object) {
                            ...
                            for (let item3 in inside3) {
                                ...
                                } else if (item3 !== '__typename') {
                                    gotInfo.push({key: key, data: inside3[item3]})
                                }
                            }

                        } else if (item !== '__typename') {gotInfo.push({key: key, data: inside2[item]})}
                    }
    ...
    for (let item in gotInfo) {
        if (!gotInfo[item].key.includes('Connection')) {
            ...
            switch (gotInfo[item].key) {
                case 'filmConnection':
                    film.push(gotInfo[item].data); break
                case 'starshipConnection':
                    starship.push(gotInfo[item].data); break
                case 'vehicleConnection':
                    vehicle.push(gotInfo[item].data); break
                case 'residentConnection':
                    resident.push(gotInfo[item].data); break
                case 'speciesConnection':
                    species.push(gotInfo[item].data); break
                case 'characterConnection':
                    character.push(gotInfo[item].data); break
                case 'personConnection':
                    person.push(gotInfo[item].data); break
                case 'planetConnection':
                    planet.push(gotInfo[item].data); break
                case 'pilotConnection':
                    pilot.push(gotInfo[item].data); break
            }
        }
    }
    if (film.length > 0) filteredInfo.push({key: 'Films connection', data: film})
